...
import { Home, List, History, LogOut } from "lucide-react";

const SidebarEtalase = () => {
    ...
    <aside className="w-20 min-h-screen bg-[#1f2937] text-white flex flex-col items-center py-6 fixed top-0 left-0 z-20">
      ...
      <nav className="flex flex-col items-center gap-6 w-full">
        <MenuItem
          ...
          active={currentPath === "/etalase"}
        />
        <MenuItem
          icon={<List size={24} />}
          label="Pesanan"
          onClick={() => navigate("/etalase/pesanan-masuk")}
          active={currentPath === "/etalase/pesanan-masuk"}
        />
        <MenuItem
          icon={<History size={24} />}
          label="Riwayat"
          onClick={() => navigate("/etalase/riwayat")}
          active={currentPath === "/etalase/riwayat"}
        />
      </nav>
