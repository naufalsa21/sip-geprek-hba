...
const DetailPesananEtalasePage = () => {
  ...
  const etalaseId = localStorage.getItem("userId");
  ...
  const handleUpdateStatus = async () => {
    try {
      ...
      if (pesanan.status === "Belum Diproses") {
        endpoint = "proses";
      } else if (pesanan.status === "Sedang Diproses") {
        endpoint = "selesaikan";
      } else {
        return;
      }
      ...
      const etalaseId = localStorage.getItem("userId");
      if (!etalaseId) {
        alert("Etalase ID tidak ditemukan. Silakan login ulang.");
        return;
      }

      await axios.patch(`${API_BASE_URL}/api/pesanan/${id}/${endpoint}`, {
        etalase_id: etalaseId,
      });

      alert(`Pesanan berhasil di-${endpoint}.`);
...
export default DetailPesananEtalasePage;
